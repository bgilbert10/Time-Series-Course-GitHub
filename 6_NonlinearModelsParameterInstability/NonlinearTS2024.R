...
# 1) 
getSymbols("GC=F", from="2001-01-03")   #gold futures price
...
grd <- merge.xts(`GC=F`$`GC=F.Adjusted`, DFII10$DFII10, 
                 T10YIE$T10YIE, DTWEXBGS, DFF$DFF,
                 all=TRUE, fill=NA, retclass="xts",join="inner")

grd <- merge.xts(`GC=F`$`GC=F.Adjusted`, DFF$DFF,
                 all=TRUE, fill=NA, retclass="xts",join="inner")
colnames(grd) <- c("gld","dff")

colnames(grd) <- c("gld","tip", "bei","twd","dff")
...
gld <- ts(na.omit(grd$gld),freq=252,start=2001)
tip <- ts(na.omit(grd$tip),freq=252,start=2017)
dff <- ts(na.omit(grd$dff),freq=252,start=2001)
...
plot(gld.setar.tip)
gld.setar.dff <- setar(gld,mL=3,mH=3,thVar = dff)

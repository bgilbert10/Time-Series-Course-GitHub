...
## test mean return of zero, Jarque-Bera test, skewness and kurtosis tests
...
# Individual stocks: SRE, BRK, DUK (Sempra, Berkshire, Duke)
...
# creates a data frame with all the daily simple returns
...
# some testing of the daily Berkshire Hathaway log returns
brkrt= na.omit(as.data.frame(100*BRK.dlrtn))
basicStats(brkrt)
mean(brkrt[,1])
var(brkrt[,1])
sqrt(var(brkrt[,1]))
...
## Visual examples
# try using BBL or RIO instead of AAPL
setwd("C:/Users/bgilbert_a/Dropbox/Econometrics/TimeSeriesCourse") # set working directory, notice slashes go forward
...
# try using forward prices at different terms for natural gas
...
# use earnings for an energy firm
